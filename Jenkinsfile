...
node {
   ...
   // Platform Service Starts
	...
	stage ('SlackNotification') {
		...
		//PlatformEnterpriseEngine version
		sh "cd /var/jenkins/jobs/$commitID/workspace/PlatformEnterpriseEngine && mvn -B help:evaluate -Dexpression=project.version | grep -e '^[^[]' > /var/jenkins/jobs/$commitID/workspace/PlatformEnterpriseEngine/version"
	    	pomversionEnterpriseEngine=readFile("/var/jenkins/jobs/$commitID/workspace/PlatformEnterpriseEngine/version").trim()	
		...
		if(pomversionService.contains("SNAPSHOT") && pomversionInsightsWebHook.contains("SNAPSHOT") 
			...
			&& pomversion.contains("SNAPSHOT") && pomversionEnterpriseEngine.contains("SNAPSHOT") 
			&& pomUI3version.contains("SNAPSHOT")){
			...
			//get artifact info (artifactID,classifier,timestamp, buildnumber,version) from maven-metadata.xml
			sh "curl -u $NexusUsername:$NexusPassword -s ${NEXUSREPO}/com/cognizant/devops/PlatformAuditEngine/${pomversionAuditEngine}/maven-metadata.xml  | grep -oP '(?<=<artifactId>).*?(?=</artifactId>)|(?<=<version>).*?(?=</version>)|(?<=<timestamp>).*?(?=</timestamp>)|(?<=<buildNumber>).*?(?=</buildNumber>)|(?<=<classifier>).*?(?=</classifier>)' | paste -sd- - | sed 's/-SNAPSHOT//g' | sed 's/--/-/g' | sed 's/\$/.jar/' > /var/jenkins/jobs/$commitID/workspace/PlatformAuditEngine/PAE_artifact"		
			...
			//get artifact info (artifactID,classifier,timestamp, buildnumber,version) from maven-metadata.xml
			sh "curl -u $NexusUsername:$NexusPassword -s ${NEXUSREPO}/com/cognizant/devops/PlatformEnterpriseEngine/maven-metadata.xml  | grep -oP '(?<=<artifactId>).*?(?=</artifactId>)|(?<=<release>).*?(?=</release>)|(?<=<timestamp>).*?(?=</timestamp>)|(?<=<buildNumber>).*?(?=</buildNumber>)|(?<=<classifier>).*?(?=</classifier>)' | paste -sd- - | sed 's/-SNAPSHOT//g' | sed 's/--/-/g' | sed 's/\$/.jar/' > /var/jenkins/jobs/$commitID/workspace/PlatformWebhookEngine/PEE_artifact"
			...
			//get artifact info (artifactID,classifier,timestamp, buildnumber,version) from maven-metadata.xml
			sh "curl -u $NexusUsername:$NexusPassword -s ${NEXUSREPO}/com/cognizant/devops/PlatformEnterpriseEngine/maven-metadata.xml  | grep -oP '(?<=<artifactId>).*?(?=</artifactId>)|(?<=<release>).*?(?=</release>)|(?<=<timestamp>).*?(?=</timestamp>)|(?<=<buildNumber>).*?(?=</buildNumber>)|(?<=<classifier>).*?(?=</classifier>)' | paste -sd- - | sed 's/-SNAPSHOT//g' | sed 's/--/-/g' | sed 's/\$/.jar/' > /var/jenkins/jobs/$commitID/workspace/PlatformEnterpriseEngine/PAE_artifact"	
