...
// All single and double quotes in this file are used in a certain format.Do not alter in any step 
   ...
   stage ('Insight_PS_Build') {
	...
	stage ('Insight_PUI2.0_NexusUpload') {		
		...
		sh "mvn deploy:deploy-file -DgeneratePom=false -DpomFile=pom.xml -Dfile=/var/jenkins/jobs/$commitID/workspace/PlatformUI2.0/app.zip -DgroupId='com.cognizant.devops' -DartifactId='PlatformUI2.0' -Dpackaging=zip -Dversion=${pomversion} -DrepositoryId=nexus -Durl=${NEXUSREPO} -DskipTests=true"
	     } 
