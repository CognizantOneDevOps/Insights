...
public class QueryCachingServiceImpl implements QueryCachingService {
	...
	private JsonObject getEsCachedResults(String requestPayload) {
		...
		try {
			...
			// Pull cypher query, start time, end time, cacheNocache variable out of json
			// If noCache= true, directly hit neo4j and return result
			// Create hash out of cypherquery
			// Query ES databasse based on hash and start and end time
			// if result present, return to UI
			// Query Neo4j get result
			// Save result with hashcode and time field in ES
			JsonParser parser = new JsonParser();
			JsonObject requestJson = parser.parse(requestPayload).getAsJsonObject();
