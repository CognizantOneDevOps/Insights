...
public class AlertEmailSubscriber {
	private static Logger log = LogManager.getLogger(AlertEmailSubscriber.class.getName());
	...
	public boolean alertEmailing(JsonObject incomingTaskMessage) {
		JsonObject statusObject = null;
		boolean result = false;

		try {
			executionId = incomingTaskMessage.get("executionId").getAsInt();
			workflowId = incomingTaskMessage.get("workflowId").getAsString();
			long startTime = System.nanoTime();
			InsightsEmailTemplates emailTemplate = workflowDAL.getEmailTemplateByWorkflowId(
					incomingTaskMessage.get(AssessmentReportAndWorkflowConstants.WORKFLOW_ID).getAsString());
			if (emailTemplate != null) {
				...
				if (!successJobs.isEmpty()) {
					...
					InsightsStatusProvider.getInstance().createInsightStatusNode(
							"AlertEmailSubscriber Email Completed ", PlatformServiceConstants.SUCCESS);
					result = true;
				}
				...
				log.debug(StringExpressionConstants.STR_EXP_ALERT_EMAIL_EXECUTION, executionId, workflowId,
						mailReportDTO.getSubject(), mailReportDTO.getMailTo(), mailReportDTO.getMailFrom(),
						processingTime, mailReportDTO.getMailBody());
			} else {
				throw new InsightsJobFailedException("Email template not found!");
			}
		} catch (InsightsJobFailedException e) {
			log.error("Workflow Detail ==== AlertEmailSubscriber Email Send failed to execute Exception ===== ", e);
			updateEmailHistoryWithStatus(
					incomingTaskMessage.get(AssessmentReportAndWorkflowConstants.EXECUTIONID).getAsLong(),
					WorkflowTaskEnum.EmailStatus.ERROR.name());
			InsightsStatusProvider.getInstance().createInsightStatusNode(
					"AlertEmailSubscriber Email Completed with error " + e.getMessage(),
					PlatformServiceConstants.FAILURE);
			
			log.error(StringExpressionConstants.STR_EXP_ALERT_EMAIL_EXECUTION, executionId, workflowId,
					mailReportDTO.getSubject(), mailReportDTO.getMailTo(), mailReportDTO.getMailFrom(), "-",
					e.getMessage());

			throw new InsightsJobFailedException("Failed to send email in AlertEmailSubscriber "+ statusObject);
		} catch (Exception e) {
			log.error("Workflow Detail ==== AlertEmailSubscriber Email Send failed to execute Exception ===== ", e);
			InsightsStatusProvider.getInstance().createInsightStatusNode(
					"AlertEmailSubscriber Email Completed with error " + e.getMessage(),
	...
	public MailReport collectInfoFromDataBase(JsonObject incomingTaskMessage,
		...
		try {
			...
			executionId = incomingTaskMessage.get(AssessmentReportAndWorkflowConstants.EXECUTIONID).getAsInt();
			...
			// attachments.put(emailHistory.getMailAttachmentName(),
			// emailHistory.getAttachmentData());
			mailReportDTO.setMailAttachments(attachments);
			return mailReportDTO;
		} catch (Exception e) {
			log.error("Workflow Detail ==== AlertEmailSubscriberEmail Error while creating MailreportDTO ===== ", e);
	...
	private MailReport updateEmailHistoryWithEmailTemplateValues(JsonObject incomingTaskMessage,
		...
		try {
			...
			InsightsReportVisualizationContainer emailHistoryConfig = workflowDAL.getEmailExecutionHistoryByExecutionId(
					incomingTaskMessage.get(AssessmentReportAndWorkflowConstants.EXECUTIONID).getAsLong());
			if (emailHistoryConfig != null) {
			...
			log.error("Workflow Detail ==== AlertEmailSubscriber Incorrect alert email format found ===== ", e);
	...
	private List<InternetAddress> createRecipientAddress(String[] recipientList) {
		...
		for (String recipient : recipientList) {
			try {
				...
				log.error("Workflow Detail ==== AlertEmailSubscriber Incorrect alert email format found ===== ", e);
				throw new InsightsJobFailedException("Incorrect alert email format found!");
			}
		}
		return recipientAddress;
	}

	public void stopAlertEmailing(InsightsOfflineAlerting alertConfig) {
		try {
			workflowDAL.updateWorkflowConfigActive(alertConfig.getWorkflowConfig().getWorkflowId(), false);
