...
public class UpshiftAssessmentTestData {
	...
	public void readUpshiftAssessmentFileAndSave(String fileName, String executionId) throws Exception {
		...
		try {
			...
			input = new FileInputStream(vsmReportFile);
			...
			workflowId = workflowType + "_" + InsightsUtils.getCurrentTimeInSeconds();
			JsonArray taskList = new JsonArray();
			JsonArray workflowList = getTaskList(workflowType);
			workflowList.forEach(task -> taskList.add(
					createTaskJson(task.getAsJsonObject().get(AssessmentReportAndWorkflowConstants.TASK_ID).getAsInt(),
			...
			InsightsWorkflowConfiguration workflowConfig = saveWorkflowConfig(workflowId, isActive, reoccurence,
					schedule, workflowStatus, workflowType, taskList, 0, emailDetailsJson, runImmediate);
	...
	public void initializeTask() {
		try {
			...
			WorkflowTaskSubscriberHandler testNodeCreation = new UpshiftAssessmentExecutionSubscriber(
					mqChannelNodeCreation);
			registry.put(getTaskId(mqChannelNodeCreation), testNodeCreation);
			WorkflowTaskSubscriberHandler testRelationCreation = new UpshiftAssessmentRelationExecutionSubscriber(
					mqChannelRelationCreation);
