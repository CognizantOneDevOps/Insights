...
@Component({
    ...
    styleUrls: ["./prediction-show-details-dialog.css"]
})
export class PredictionShowDetailsDialog implements OnInit {
    ...
    constructor(public dialogRef: MatDialogRef<PredictionShowDetailsDialog>,
        @Inject(MAT_DIALOG_DATA) public data: any,
        private mlwizardService: MLWizardService, public messageDialog: MessageDialogService,
    ...
    async getPrediction() {
        ...
        this.predictionlist = await this.mlwizardService.getPredictions(this.usecase, this.model_id);
        if (this.predictionlist != null && this.predictionlist.status == "success") {
    ...
    exportToExcel() {
        this.sheetName = (this.model_id.length > 31)? this.model_id.slice(0,28)+'..': this.model_id;
        const workSheet = XLSX.utils.json_to_sheet(this.predictionDetailedDatasource.data);
