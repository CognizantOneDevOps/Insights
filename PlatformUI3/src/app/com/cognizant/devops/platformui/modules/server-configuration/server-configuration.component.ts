...
export class ServerConfigurationComponent implements OnInit {
   ...
   getconfigDataParsed(data) {
    ...
    if (data != undefined) {
      for (let configDatakey of Object.keys(data)) {
        ...
        number =number +1 ;
        if (typeof (data[configDatakey]) == 'object' && configDatakey != 'trustedHosts') {
        ...
        } else if (configDatakey == 'trustedHosts') {
  ...
  getServerConfigItems(filtername: any) {
    if (filtername == 'object') {
      return this.ServerConfigItems.filter(item => (item.type == filtername   && item.key != 'trustedHosts' && item.key != 'vault'));
    } else if (filtername == 'trustedHosts') {
      return this.ServerConfigItems.filter(item => item.key == 'trustedHosts');
    }  else {
  ...
  async saveData(configDetails) {
    ...
    if(this.validateServerConfigData(configDetails)) {
      for (let configParamData of this.ServerConfigItems) {
        if (configParamData.key != "trustedHosts" && configParamData.type == "object") {
         this.item = {};
       ...
       } else if (configParamData.key != "trustedHosts" && configParamData.type != "object") {
         this.updatedConfigParamdata[configParamData.key] = this.checkDatatype(configParamData.value);
       } else if (configParamData.key == "trustedHosts") {
         this.updatedConfigParamdata["trustedHosts"] = JSON.parse(configParamData.value);
       }
     }
