...
@Component({
  ...
  styleUrls: ['./add-workflow-task.component.css', './../../home.module.css']
})
export class AddWorkflowTaskComponent implements OnInit {
  ...
  async loadData() {
    ...
    this.managementService.getWorkFlowType()
      .then((response) => {
  ...
  async validateTaskDetails() {
    ...
    if (
      ...
      isValidated = false;
      messageDialogText = 'Please enter valid description, it contains only alphanumeric character and underscore.';
    }
    else if (!checknameCn) {
      isValidated = false;
      messageDialogText = 'Please enter valid component name, it contains only alphabetical character and period.';
    }
    else if (!checknameMQ) {
      isValidated = false;
      messageDialogText = 'Please enter valid MqChannel, it contains only alphabetical character, period(.) and underscore.';
    }
    else {
  ...
  validateUnique() {
    ...
    for (let task of this.taskData) {
      if (task.description === this.description && task.componentname === this.componentName) {
        return false;
      }
    }
    return true;
  ...
  refresh() {
    ...
    this.router.navigate(["InSights/Home/workflow-task-management"], navigationExtras);
  }
