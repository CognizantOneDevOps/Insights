...
public class PdfTableUtil {
	...
	private static final Logger log = LoggerFactory.getLogger(PdfTableUtil.class.getName());
	...
	public byte[] generateTableContent(List<Map> assetList, String pdfName) throws IOException, JAXBException, UnrecoverableKeyException, KeyStoreException, NoSuchAlgorithmException, CertificateException{
		...
		//Table rows
		generateDynamicRows(assetResults, headerRow, table,page);
	...
	private void generatePageStaticContent(PDDocument doc, float margin, PDPage page, float yStartNewPage,
		...
		Cell<PDPage> cell = row.createImageCell(25f, ImageUtils.readImage(new File(classLoader.getResource("sign/Insights.png").getFile())));
	...
	private List<String[]> formatAsset(List<Map> assetList) {
		...
		for(int i=0;i<assetList.size();i++){
			...
			assetArray[6] = checkEmpty(String.valueOf(assetList.get(i).get("env")));
			assetArray[7] = checkEmpty(String.valueOf(assetList.get(i).get("updatedby")));
	 ...
	 * @throws IOException
	 */
	private void generateDynamicRows(List<String[]> assetResults, Row<PDPage> headerRow, BaseTable table, PDPage page) throws IOException {
		Cell<PDPage> cell;
	...
	public PDDocument generateStaticContent() throws IOException, JAXBException {
		...
		Unmarshaller jaxbUnmarshaller = jaxbContext.createUnmarshaller();
		PdfStaticContent pdfStaticContent = (PdfStaticContent) jaxbUnmarshaller.unmarshal(staticFile);
		System.out.println(pdfStaticContent);
		yStart = yStart -100;
		// draw page title
		PDPageContentStream cos = new PDPageContentStream(doc, page);
	...
	public byte[] generateCypherReport(Set<String> headerList, List<String> rowValueList,String pdfName) {
		...
		try{
			...
			//Table rows
			generateDynamicRows(rowList, headerRow, dataTable,page);
			/*dataTable.draw();
	...
	public byte[] generateLedgerReport(List<Map> ledgerMap, String pdfName) {
		...
		try{
			...
			System.out.println("size of map ---"+ledgerMap.size());
			for(int i=0;i<ledgerMap.size();i++){
				if(i>0){
					PDPage page1 = addNewPage(doc);
					BaseTable dataTable = new BaseTable(page1.getMediaBox().getHeight() - (2 * margin),
					...
					//System.out.println("assse--"+assetList);
					List<String[]> assetResults = formatAsset(assetList);
					Row<PDPage> headerRow = generateHeader(dataTable);
					generateDynamicRows(assetResults, headerRow, dataTable,page1);
				}else{
					BaseTable dataTable = new BaseTable(page.getMediaBox().getHeight() - (2 * margin)-120,
							yStartNewPage, bottomMargin, tableWidth, margin, doc, page, true, drawContent);
					List<Map> assetList = (List<Map>) ledgerMap.get(i).get("data");
					//System.out.println("assse--"+assetList);
					List<String[]> assetResults = formatAsset(assetList);
					Row<PDPage> headerRow = generateHeader(dataTable);
					generateDynamicRows(assetResults, headerRow, dataTable,page);
				}
