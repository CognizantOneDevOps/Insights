...
public class AuditLedgerReport extends AuditReportStrategy{
							...
							for(JsonElement element : rowArray) {
								...
								for (Map.Entry<String, JsonElement> property : msgData.entrySet()) {
									if(property.getKey().contains("AssetID")){
										...
										String url = ApplicationConfigProvider.getInstance().getInsightsServiceURL()+"/PlatformAuditService/traceability/getAssetHistory?assetId="+assetId;
										String ledgerresponse = SystemStatus.jerseyGetClientWithAuthentication(url, "admin", "admin", null);
										...
										if("success".equalsIgnoreCase(jp.get("status").getAsString())){
											...
											ledgerList.add(formatAssetId(ledgerresponse));
	 ...
	 */
	private Map formatAssetId(String ledgerresponse) {
		JsonParser jsonParser = new JsonParser(); 
