...
export class PredictionComponent implements OnInit {
  ...
  displayedColumnsForClassification = [
    "radio",
    "model_id",
    "auc",
    "logloss",
  ...
  infoURL =
    "https://h2o-release.s3.amazonaws.com/h2o/rel-xu/5/docs-website/h2o-docs/performance-and-prediction.html";
  ...
  getLeaderBoard() {
    ...
    this.mlwizardService.getLeaderboard(this.usecaseid).then((event) => {
      if (event.status == "success") {
        ...
        const dialog = this.messageDialog.showApplicationsMessage(
          "Unable to fetch leaderboard data. You may need to re-run the usecase <b>" +
            this.usecaseid +
            "</b>.",
          "ERROR"
        );
  ...
  onSaveMOJO() {
    const dialogRef = this.messageDialog.showConfirmationMessage(
      "Save ",
      "Do you want to download MOJO <b>" +
        this.selectedModel.model_id +
        "</b> ? <br/>" +
        " <b>Please Note:</b> This will be final MOJO to be deployed and it cannot be altered later. ",
      this.selectedModel.model_id,
      "ALERT",
      "30%"
    );
    dialogRef.afterClosed().subscribe((result) => {
      if (result == "yes") {
        ...
        this.mlwizardService
          .saveMOJO(this.usecaseid, this.selectedModel.model_id)
          .then((event) => {
            if (event.status == "success") {
              ...
              this.showThrobber = false;
              this.messageDialog.openSnackBar(
                "Couldn't download the MOJO. Please try again.",
                "error"
              );
            }
          });
  ...
  isMLSelected() {
    if (this.targetML == null) {
      this.messageDialog.openSnackBar(
        "Please choose an algorithm name to continue",
        "error"
      );
      return false;
    } else return true;
