...
@Component({
  ...
  styleUrls: ["./webhook.component.scss", "./../home.module.scss"],
})
          ...
          this.webhookService
            ...
            .then(function (response) {
              if (response.status == "success") {
                self.messageDialog.openSnackBar("<b>" + self.webhookName + "</b> saved successfully.", "success");
                self.list();
              } else if (response.message === "Webhook name already exists") {
                self.messageDialog.openSnackBar("<b>" + self.webhookName +
                  "</b> already exists. Please try again with a new name.", "error");
              } else if (response.message == "Incorrect Response Template") {
                self.messageDialog.openSnackBar("Incorrect Response Template.", "error");
              } else {
                self.messageDialog.openSnackBar("Failed to save the webhook.Please check logs.", "error");
              }
            });
        }
      });
          ...
          this.webhookService
            ...
            .then(function (data) {
              if (data.status == "success") {
                self.messageDialog.openSnackBar("Changes made to <b>" + self.webhookName + "</b> are saved successfully.", "success");
                self.list();
                self.radioRefresh = false;
              } else {
                self.messageDialog.openSnackBar("Failed to save Webhook.Please check logs.", "error");
              }
            });
        }
      });
  ...
  actionSubscribeOrUnsubscribe(status) {
    ...
    this.webhookService
      ...
      .then(function (data) {
        if (data.status == "success") {
          if (status == true) {
            self.messageDialog.openSnackBar("You have subscribed to <b> " +
              self.selectedWebhook.webhookName +
              "</b> successfully. You may unsubscribe by clicking the Unsubscribe icon later.", "success");
          } else {
            self.messageDialog.openSnackBar("You have unsubscribed to <b> " +
              self.selectedWebhook.webhookName +
              "</b> successfully. You may subscribe by clicking the Subscribe icon later.", "error");
          }
          self.list();
          self.radioRefresh = false;
        } else {
          self.messageDialog.openSnackBar("Webhook Subscribe Failed!", "error");
        }
      });
  }
    ...
    dialogRef.afterClosed().subscribe((result) => {
      if (result == "yes") {
        self.webhookService
          ...
          .then(function (data) {
            self.list();
            self.messageDialog.openSnackBar("Deleted Successfully", "success");
          })
          .catch(function (data) {
            self.messageDialog.openSnackBar("Something wrong with Service.Please try again.", "error");
          });
