...
export interface IModelManagementService {
  loadUsecaseDetails(): Promise<any>;
  deleteUsecase(usecase: string): Observable<any>;
}
...
export class ModelManagementService implements IModelManagementService {
  ...
  loadUsecaseDetails(): Promise<any> {
    ...
    return restHandler.get("GET_USECASES");
  }

  deleteUsecase(usecase: string): Observable<any> {
    var restHandler = this.restCallHandlerService;
    return restHandler.postWithParameter("DELETE_USECASE", {
      usecase: usecase,
    });
  }
  ...
  usecaseStateUpdate(usecaseConfig: string) {
    return this.restCallHandlerService
      .postWithData("UPDATE_USECASE_STATE", usecaseConfig, {
