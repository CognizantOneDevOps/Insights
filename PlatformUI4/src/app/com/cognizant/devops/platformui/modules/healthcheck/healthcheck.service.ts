...
export class HealthCheckService implements IHealthCheckService {
  ...
  loadHealthConfigurations(
    ...
    var restHandler = this.restCallHandlerService;
    return restHandler.get("HEALTH_TOOL", {
      tool: toolName,
  ...
  downloadLog(fileName): Observable<Blob> {
    ...
    return this.httpClient.get("/PlatformService/traceability/getReportLog", {
  ...
  getAgentFailureDetails(
    ...
    var restHandler = this.restCallHandlerService;
    return restHandler.get("AGENTS_FAILURE_DETAILS", {
      tool: toolName,
      category: toolCategory,
      agentId: agentId,
    });
  ...
  updateNotification(setStatusRequestJson: string) {
    return this.restCallHandlerService
      .postWithData("UPDATE_NOTIFICATION_STATUS", setStatusRequestJson, {
