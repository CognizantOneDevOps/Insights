...
@Component({
  ...
  styleUrls: ["add-task.scss", "./../../home.module.scss"],
})
export class AddTasksDialog {
  ...
  constructor(
    public reportmanagemnetService: ReportManagementService,
    private dragulaService: DragulaService,
    public dialogRef: MatDialogRef<AddTasksDialog>,
    ...
    const renumbering = (
      ...
      // Renumbering source
      sourceModel
        .filter((o) => o.description !== item.description)
        .forEach(
          (_item) =>
            (_item.sequence =
    ...
    this.subs.add(
      dragulaService
        .dropModel<Iitem>("TASKS")
        .subscribe(
          ({ name, el, target, source, sourceModel, targetModel, item }) => {
  ...
  async getListOfTasks() {
    this.responseOfTasklist = await this.reportmanagemnetService.getTasksList(
      "Report"
    );
