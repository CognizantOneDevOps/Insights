...
@Component({
  ...
  styleUrls: ["./businessmapping.component.scss", "./../../home.module.scss"],
})
export class BusinessMappingComponent implements OnInit {
  ...
  async getToolInfo() {
    try {
      var dictResponse =
        await this.businessMappingService.loadToolsAndCategories();
  ...
  displayToolMappingDetail() {
    ...
    self.businessMappingService
      .getToolMapping(this.selectedLabel.toolName)
      .then(function (usersMappingResponseData) {
        if (usersMappingResponseData.status == "success") {
          if (usersMappingResponseData.data != undefined) {
            usersMappingResponseData.data = self.clubProperties(
              usersMappingResponseData.data,
              true
            );
  ...
  async loadToolProperties(selectedTool) {
    try {
      ...
      if (
        this.masterToolPropertiesData.data.length > 1 &&
        this.masterToolPropertiesData.data != undefined &&
        this.masterToolPropertiesData.status == "success"
      ) {
        if (this.actionType == "edit") {
          ...
          for (let masterData of this.masterToolPropertiesData.data) {
            ...
            if (checkvalue) {
              ...
              if (existingKeys.indexOf(masterData) == -1) {
                let toolMappingLabel = new ToolLabelMapping(
                  masterData,
                  masterData,
                  "",
                  "a",
                ...
                if (this.unwantedLabel.indexOf(masterData) == -1) {
                  if (this.additionalProperties.indexOf(masterData) > -1) {
                    toolMappingLabel = new ToolLabelMapping(
                      masterData,
                      masterData,
                      this.selectedMappingTool[masterData],
                      "a",
                      false
                    );
                  } else {
          ...
          for (var key in this.masterToolPropertiesData.data) {
            var checkvalue = regex.test(
              this.masterToolPropertiesData.data[key]
            );
            if (checkvalue) {
              // Skip Key changes
            } else {
              let toolMappingLabel;
              if (
                this.additionalProperties.indexOf(
                  this.masterToolPropertiesData.data[key]
                ) > -1
              ) {
                toolMappingLabel = new ToolLabelMapping(
      ...
      this.toolPropertyDataSource = new MatTableDataSource(
        this.toolPropertyDataSource
      );
  ...
  editData() {
    ...
    this.subHeading =
      "Edit Business Mapping Label for " +
      this.selectedMappingTool.businessmappinglabel;
  }
  ...
  saveData() {
    ...
    if (numSelected == 0) {
      this.messageDialog.showApplicationsMessage(
        "Please select at least one Tool Property to create a Label",
        "WARN"
      );
    } else if (this.label == "" || this.label == undefined) {
      this.messageDialog.showApplicationsMessage(
        "Mapping Label value is <b>MANDATORY</b> it should not be empty",
        "WARN"
      );
    } else {
      //Validate Label
      ...
      selectedData.forEach((row) => {
          ...
          validationMessage =
            "Value should not be empty for propety key <b>" + row.key + "</b>";
          } else {
          let toolMappingLabelSelected = new ToolLabelMapping(
            row.id,
            row.key,
            row.value,
            this.label,
      ...
      if (validationMessage == "") {
        if (this.actionType == "add") {
          ...
          this.toolPropertyList["categoryName"] =
            this.selectedLabel.categoryName;
        ...
        this.messageDialog.openSnackBar(validationMessage, "error");
        this.selection.clear();
      }
    }
  }
  ...
  public validateData(validationMessage): any {
    if (validationMessage == "") {
      ...
      if (format.test(str) == true) {
        validationMessage =
          "Please check blank space and special charecters are not allowed, It should be like Label1:Label2:Label3";
      }
      if (this.actionType == "add") {
        for (let data of this.toolDataSource.data) {
          if (data.businessmappinglabel == this.label) {
            validationMessage =
              "Mapping Label already exists for tool <b>" +
              this.selectedLabel.toolName +
              " </b>.";
          }
          if (
            data.propertiesString == this.toolPropertyList["propertiesString"]
          ) {
            validationMessage =
              "Properties with same name and value are already exists for tool <b>" +
        ...
        for (let data of this.toolDataSource.data) {
          if (data.businessmappinglabel == this.label && data.propertiesString == this.toolPropertyList['propertiesString']) {
            validationMessage = "Properties with same name and value <b> " + this.toolPropertyList['propertiesString'] + " </b> already exists for tool <b>" + this.selectedTool.toolName + " </b>.Please edit necessary values if applicable";
          }
        }
      }
    }
  ...
  callEditOrSaveDataAPI(toolMappingParameter: any) {
    ...
    var title =
      this.actionType == "add"
        ? "Save Business Mapping Label"
        : " Edit Business Mapping Label";
    var dialogmessage =
      this.actionType == "add"
        ? "Are you sure do you want to save changes to the Business Mapping Label <b> " +
          this.label +
          " </b>?"
        : "Please note: The changes will be applied from next data collection. <br> Are you sure do you want to save changes to the Business Mapping Label <b> " +
          this.label +
          " </b>?";
    ...
    dialogRef.afterClosed().subscribe((result) => {
      if (result == "yes") {
        ...
        if (this.actionType == "add") {
          this.businessMappingService
            ...
            .then(function (saveResponsedata) {
              if (saveResponsedata.status == "success") {
                ...
                self.messageDialog.openSnackBar(
                  "Unable to save the Business Mapping Label <b> " +
                    self.label +
                    " </b>. " +
          ...
          this.businessMappingService
            .editToolMapping(toolMappingParameter)
            .then(function (editResponsedata) {
              if (editResponsedata.status == "success") {
                self.messageDialog.openSnackBar(
                  "The changes you made to the Business Mapping Label <b> " +
                    self.label +
                    " </b> have been updated successfully.",
                  "success"
                );
              } else {
                self.messageDialog.openSnackBar(
                  "Unable to edit Business Mapping Label <b> " +
                    self.label +
                    " </b>. " +
                    editResponsedata.message,
                  "error"
                );
              }
              self.displayToolMappingDetail();
  ...
  deleteMapping() {
    var self = this;
    if (
      this.selectedMappingTool.uuid != undefined ||
      this.selectedMappingTool.uuid != ""
    ) {
      var title = "Delete Business Mapping Label";
      var dialogmessage =
        "<b>PLEASE NOTE:</b><br><br>Deleting Business Mapping Label <b>" +
        this.selectedMappingTool.businessmappinglabel +
        "</b>, this action <b>CANNOT BE UNDONE.</b><br><br>Once Business Mapping Label<b> " +
        this.selectedMappingTool.businessmappinglabel +
        "</b> is deleted, data related to  will be retained in the previously gathered data. However, if you create a new Business Mapping Label with the same name, it may impact other functionalities.</b><br><br>Are you sure, do you want to Delete Business Mapping Label <b>" +
        this.selectedMappingTool.businessmappinglabel +
        "</b>?";
      const dialogRef = this.messageDialog.showConfirmationMessage(
        title,
      ...
      dialogRef.afterClosed().subscribe((result) => {
        if (result == "yes") {
          this.businessMappingService
            .deleteToolMapping(this.selectedMappingTool.uuid)
            .then(function (deleteResponsedata) {
              if (deleteResponsedata.status == "success") {
                self.messageDialog.openSnackBar(
                  "Label delete Successfully ",
                  "success"
                );
              } else {
                self.messageDialog.openSnackBar(
                  "Unable to delete label " + deleteResponsedata.message,
                  "error"
                );
              }
              self.displayToolMappingDetail();
      ...
      self.messageDialog.openSnackBar(
        "Unable to delete Label , Please tray again later",
        "error"
      );
    }
  }
