...
@Component({
  ...
  styleUrls: ['./relationship-builder.component.scss', './../home.module.scss']
})
export class RelationshipBuilderComponent implements OnInit {
  ...
  relationDelete() {
    ...
    const dialogRef = this.messageDialog.showConfirmationMessage(title, dialogmessage, this.selectRelation.relationName, "ALERT", "34%");
    dialogRef.afterClosed().subscribe(result => {
      if (result == 'yes') {
        var finalJson = {};
        ...
        this.relationshipBuilderService.deleteCorrelation(correlationJson).then(
          (corelationResponse2) => {
            if (corelationResponse2.status == "success") {
              ...
              this.messageDialog.openSnackBar(dialogmessage, "success");
            }
          });
        this.Refresh();
      }
  ...
  saveData(newName) {
    ...
    if (newName == "" || newName == undefined || !checkname) {
      newName = undefined;
      this.messageDialog.openSnackBar("Please enter valid name, and it should contain only alphanumeric characters and underscore ", "error");
    }
    else if ((this.selectedDestinationLabel || this.selectedSourceLabel) == undefined) {
      this.messageDialog.openSnackBar("<b>Label Name cannot be null</b>", "error");
    }
    else if ((this.fieldSourceProp || this.fieldDestProp) == []) {
      this.messageDialog.openSnackBar("<b>Fields value cannot be null</b>", "error");
    }
    else {
      ...
      if (this.count == 0) {
        ...
        const dialogRef = this.messageDialog.showConfirmationMessage(title, dialogmessage, this.selectRelation, "ALERT", "33%");
        dialogRef.afterClosed().subscribe(result => {
          if (result == 'yes') {
            //DESTINATION
            ...
            this.relationshipBuilderService.saveCorrelationConfig(addMappingJson).then(
              (corelationResponse2) => {
                if (corelationResponse2.status == "success") {
                  ...
                  this.messageDialog.openSnackBar(corelationResponse2.message, "error");                  this.isSelfRelation = false;
                }
              });
        ...
        this.messageDialog.openSnackBar(dialogmessage, "error");
        this.count = 0;
        newName = undefined;
      }
      else {
