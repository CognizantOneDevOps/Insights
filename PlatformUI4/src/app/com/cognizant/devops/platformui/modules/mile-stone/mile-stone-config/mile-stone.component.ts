...
@Component({
  ...
  styleUrls: ["./../../outcome/outcome-config/outcome.component.scss", "./../../home.module.scss"],
})
export class MileStoneComponent implements OnInit {
  ...
  getStartDate(event: MatDatepickerInputEvent<Date>) {
    this.start = this.dataShare.convertDateToSpecificDateFormat(
  ...
  getEndDate(event: MatDatepickerInputEvent<Date>) {
    this.end = this.dataShare.convertDateToSpecificDateFormat(
      new Date(event.value),
      "yyyy-MM-dd'T'HH:mm:ss'Z'"
    );
  }
      ...
      dialogRef.afterClosed().subscribe((result) => {
        if (result == "yes") {
          this.mileStoneService
            .saveMileStoneConfig(requestObj)
            .then(function (response) {
              ...
              if (response.status === "success") {
                ...
                self.router.navigateByUrl("InSights/Home/fetchMileStone", {
  ...
  redirect() {
    this.router.navigate(["InSights/Home/fetchMileStone"], {
      skipLocationChange: true,
    });
  }
}
