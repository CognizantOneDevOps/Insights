...
@Component({
  ...
  styleUrls: ["./server-configuration.component.scss", "./../home.module.scss"],
})
export class ServerConfigurationComponent implements OnInit {
  ...
  getconfigDataParsed(data) {
    ...
    if (data != undefined) {
      for (let configDatakey of Object.keys(data)) {
        ...
        var isRequiredField = this.requiredProperties.find(
        ...
        if (
          ...
          configDatakey != "trustedHosts" &&
          configDatakey != "applicationLogLevel"
        ) {
          for (let configinnerDatakey of Object.keys(value)) {
            number = number + 1;
            let serverConfigChild = new ServerConfigItem();
            var isRequiredChildField = this.requiredProperties.find(
  ...
  getServerConfigItems(filtername: any) {
    if (filtername == "object") {
      return this.ServerConfigItems.filter(
        (item) =>
          item.type == filtername &&
          item.key != "applicationLogLevel" &&
          item.key != "trustedHosts" &&
          item.key != "vault"
      );
    } else if (filtername == "textArea") {
      return this.ServerConfigItems.filter(
        (item) =>
          item.key == "trustedHosts" || item.key == "applicationLogLevel"
      );
    } else {
  ...
  async saveData(configDetails) {
    ...
    if (this.validateServerConfigData(configDetails)) {
      for (let configParamData of this.ServerConfigItems) {
        ...
        if (
          configParamData.key != "trustedHosts" &&
          configParamData.key != "applicationLogLevel" &&
          ...
          this.updatedConfigParamdata[configParamData.key] = JSON.parse(
            configParamData.value
          );
        }
      }
      if (
        this.updatedConfigParamdata["autheticationProtocol"] !=
          this.serverConfigData.autheticationProtocol ||
        this.updatedConfigParamdata["grafana"]["grafanaEndpoint"] !=
          this.serverConfigData.grafana.grafanaEndpoint
      ) {
        message =
          "<br/> <b>Please note:</b> You have changed some of the basic properties of server config which needs server restart. ";
      }
      console.log(this.updatedConfigParamdata);
      var dialogmessage =
        "Are you sure you want to save Server Config details ?" + message;
        ...
        if (result == "yes") {
          this.serverconfigService
            .saveServerConfigurations(
              JSON.stringify(this.updatedConfigParamdata)
            )
            .then(function (response) {
              if (response.status == "success") {
                const dialogref = self.messageDialog.showApplicationsMessage(
                  "Server config details saved successfully.",
                  "SUCCESS"
                );
  ...
  validateServerConfigData(configDetails) {
    if (
      configDetails.graph_endpoint.length == 0 ||
      configDetails.graph_authToken.length == 0 ||
      configDetails.graph_boltEndPoint.length == 0
    ) {
      this.messageDialog.openSnackBar("Please fill Neo4j details. ", "error");
    } else if (
      configDetails.grafana_grafanaEndpoint.length == 0 ||
      configDetails.grafana_grafanaDBEndpoint.length == 0
    ) {
      this.messageDialog.openSnackBar("Please fill Grafana details. ", "error");
    } else if (
      configDetails.postgre_insightsDBUrl.length == 0 ||
      configDetails.postgre_grafanaDBUrl.length == 0 ||
      configDetails.postgre_password.length == 0 ||
      configDetails.postgre_userName.length == 0
    ) {
      this.messageDialog.openSnackBar("Please fill Postgre details. ", "error");
    } else if (
      configDetails.messageQueue_host.length == 0 ||
      configDetails.messageQueue_user.length == 0 ||
      configDetails.messageQueue_password.length == 0
    ) {
      this.messageDialog.openSnackBar(
        "Please fill Message Queue Config details. ",
        "error"
      );
    } else if (
      configDetails.trustedHosts.length == 0 ||
      JSON.parse(configDetails.trustedHosts).length == 0
    ) {
      this.messageDialog.openSnackBar(
        "Please fill trusted hosts details, it cannot be empty. ",
        "error"
      );
    } else if (configDetails.insightsServiceURL.length == 0) {
      this.messageDialog.openSnackBar(
        "Please fill insightsServiceURL, it cannot be empty. ",
        "error"
      );
    } else if (configDetails.autheticationProtocol.length == 0) {
      this.messageDialog.openSnackBar(
        "Please fill authentication protocol. ",
        "error"
      );
    } else if (
      !this.authenticationProtocolList.find(
        (element) => element === configDetails.autheticationProtocol
      )
    ) {
      this.messageDialog.openSnackBar(
        "Please enter a valid authentication protocol. ",
        "error"
      );
    ...
    for (let configParamData of this.ServerConfigItems) {
      if (this.serverConfig.hasOwnProperty(configParamData.key)) {
        this.configAbbr[configParamData.key] =
          this.serverConfig[configParamData.key];
      } else {
  ...
  showErrorLogs(message) {
    if (message.includes("Vault_sealed")) {
      this.messageDialog.openSnackBar(
        " Vault is sealed, Please unsealed it and try again. ",
        "error"
      );
    } else if (message.includes("ConnectException")) {
      this.messageDialog.openSnackBar(
        "Unable to connect to Vault server, Please restart Vault server. ",
        "error"
      );
    } else if (message.includes("Error while saving server config file")) {
      this.messageDialog.openSnackBar(
        "Failed to save server config details, Please check logs.",
        "error"
      );
    } else {
      this.messageDialog.openSnackBar(
        "Failed to load server config details, Please check logs.",
        "error"
      );
    }
  }
