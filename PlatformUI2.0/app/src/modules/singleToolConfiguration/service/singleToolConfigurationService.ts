...
module ISightApp {
  export interface ISingleToolConfigService {
    saveToolsConfiguration(toolsJSON: Object): ng.IPromise<any>;
    readToolsConfiguration(toolName: string, toolCategory: string): ng.IPromise<any>;
    downloadAgentConfig(toolName: string, toolCategory: string, toolAgentId: number): ng.IPromise<any>;
    deleteToolsConfig(toolName: string, toolCategory: string, agentId: number): ng.IPromise<any>;
    readToolsLayoutJson(toolName: string, toolCategory: string): ng.IPromise<any>;
    readTimeZonelist(): Object;
    readToolsConfigurationGlobal(): ng.IPromise<any>;
    readToolsDataList();
    getCategoryName(category: string): string;
    getCategoryAbbreviatedName(category: string): string;
    getToolName(category: string, tool: string): string;
  }
  export class SingleToolConfigService implements ISingleToolConfigService {
    static $inject = ['$q', '$resource', '$cookies', 'restEndpointService'];
    constructor(private $q: ng.IQService, private $resource, private $cookies, private restEndpointService: IRestEndpointService) {
    }
    saveToolsConfiguration(toolsJSON: Object): ng.IPromise<any> {
    ...
    getToolName(category: string, tool: string): string {
      ...
      for (let tool in toolsData) {
        if (toolsData[tool].category === categoryName) {
          var toolsValueArray = toolsData[tool].tools;
