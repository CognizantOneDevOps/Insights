...
let Chaincode = class {
  ...
  async Instantiate(stub, args, thisClass) {
    ...
    // ==== Input sanitation ====
        ...
        //below conditions are removed because while processing changelogs, if datamodel got modified in between the changelogs, number/name of properties may vary
        ...
        try{
                ...
                console.log('asset exists==',assetState);
                console.log('tostring',assetState.toString());
                        ...
                        if(assetState.timestamp==timestamp){
                                ...
                                for(var props in tool){
                                        if(typeof(tool[props])=="object"){
                                                ...
                                                console.log('type if=',exp);
                                        }
                                        else{
                                                exp = tool[props]!=assetState[props]
                                                console.log('exp else==',exp);
                                        }
                                        if(exp){
                                                updateFlag = 1
                                                break
                ...
                if(updateFlag==0){
                        ...
                        console.log('dateAsset==',dateAsset);
        ...
        if(inputObj.length!=ledgerObj.length)
                ...
                for(var elem in inputObj){
                        if(JSON.stringify(inputObj[elem])!=JSON.stringify(ledgerObj[elem])){
                                console.log('condition satisfied=',inputObj[elem]!=ledgerObj[elem]);
                                return false
                        }
                }
                console.log('nothing satisfied returning true');
                return true
        }
  }
