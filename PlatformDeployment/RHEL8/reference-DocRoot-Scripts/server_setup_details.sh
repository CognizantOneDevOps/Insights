...
#Set insights configuration for postgres, rabbitmq, insights, and grafana
echo "Configuring Insights: Grafana(adminUserName), postgre(userName), RabbitMQ(user), Neo4J which will create in server-config.json :"
#Postgres grafana user credentials using jq
echo -n "Grafana Username created in Postgre:"
read POSTGRE_USER_NAME
echo "Grafana Username's Password created in Postgre:"
