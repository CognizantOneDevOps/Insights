...
sudo tar -zxf Python-3.7.4.tgz && cd Python-3.7.4 && sudo yum install gcc -y && sudo yum install openssl-devel -y && sudo yum install bzip2-devel -y  && sudo yum install libffi-devel -y && sudo ./configure --prefix=/opt/
./configure
make altinstall
