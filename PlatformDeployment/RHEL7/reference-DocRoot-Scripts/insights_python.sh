...
sudo mkdir python && cd python && sudo wget https://www.python.org/ftp/python/2.7.11/Python-2.7.11.tgz
sudo tar -zxf Python-2.7.11.tgz && cd Python-2.7.11 && sudo yum install gcc -y && sudo ./configure --prefix=$INSIGHTS_APP_ROOT_DIRECTORY
sudo make install && cd .. && sudo wget https://bootstrap.pypa.io/get-pip.py
python --version
