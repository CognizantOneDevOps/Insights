...
sudo cp $INSIGHTS_OFFLINE_ARTIFACTS_DIRECTORY/Postgres/pgdg.noarch.rpm .
...
sudo rpm -ivh pgdg.noarch.rpm
#sudo cp $INSIGHTS_OFFLINE_ARTIFACTS_DIRECTORY/Postgres/postgres_dependencies.zip .
...
#sudo unzip postgres_dependencies.zip
sudo cp -R $INSIGHTS_OFFLINE_ARTIFACTS_DIRECTORY/Postgres/postgres_dependencies .
cd postgres_dependencies
sudo rpm -ivh *.rpm
sudo /usr/pgsql-9.5/bin/postgresql95-setup initdb
sudo systemctl enable postgresql-9.5.service
sudo chkconfig postgresql-9.5 on
sudo cp $INSIGHTS_OFFLINE_ARTIFACTS_DIRECTORY/Postgres/pg_hba.conf .
