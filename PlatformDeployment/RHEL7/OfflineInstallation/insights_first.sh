...
echo "Enter INSIGHTS_OFFLINE_ARTIFACTS_DIRECTORY Offline Source directory example: /home/ec2-user/Offline_Installation"
read INSIGHTS_OFFLINE_ARTIFACTS_DIRECTORY

# Remove trailing // - INSIGHTS_OFFLINE_ARTIFACTS_DIRECTORY
OADS_HOME=$INSIGHTS_OFFLINE_ARTIFACTS_DIRECTORY
oads_length=${#OADS_HOME}
oads_last_char=${OADS_HOME:oads_length-2:2}

if [[ $hds_last_char == "//" && $ads_last_char == "//" ]]; then
  INSIGHTS_OFFLINE_ARTIFACTS_DIRECTORY=${OADS_HOME:0:oads_length-2};
fi

# If INSIGHTS_OFFLINE_ARTIFACTS_DIRECTORY directory doesn't exists
if [ ! -d "$INSIGHTS_OFFLINE_ARTIFACTS_DIRECTORY" ]; then
  # Control will enter here if $DIRECTORY doesn't exist.
  echo "INSIGHTS_OFFLINE_ARTIFACTS_DIRECTORY $INSIGHTS_OFFLINE_ARTIFACTS_DIRECTORY doesn't exist"
  exit
fi

echo INSIGHTS_OFFLINE_ARTIFACTS_DIRECTORY $INSIGHTS_OFFLINE_ARTIFACTS_DIRECTORY
export INSIGHTS_OFFLINE_ARTIFACTS_DIRECTORY=$INSIGHTS_OFFLINE_ARTIFACTS_DIRECTORY
sudo echo INSIGHTS_OFFLINE_ARTIFACTS_DIRECTORY=$INSIGHTS_OFFLINE_ARTIFACTS_DIRECTORY | sudo tee -a /etc/environment
sudo echo "export" INSIGHTS_OFFLINE_ARTIFACTS_DIRECTORY=$INSIGHTS_OFFLINE_ARTIFACTS_DIRECTORY | sudo tee -a /etc/profile
sudo chmod -R 777 "$INSIGHTS_OFFLINE_ARTIFACTS_DIRECTORY"
...
# Remove trailing // - INSIGHTS_HOME_ROOT_DIRECTORY and INSIGHTS_APP_ROOT_DIRECTORY
...
cd INSIGHTS_HOME
#sudo cp $INSIGHTS_OFFLINE_ARTIFACTS_DIRECTORY/First/InSightsConfig.zip .
...
sudo cp -R $INSIGHTS_OFFLINE_ARTIFACTS_DIRECTORY/First/InSightsConfig/.InSights/ .
...
source /etc/environment
source /etc/profile
myextip=$(ip route get 1.2.3.4 | awk '{print $7}')
echo $myextip
...
sudo cp $INSIGHTS_OFFLINE_ARTIFACTS_DIRECTORY/First/json_processor/jq .
