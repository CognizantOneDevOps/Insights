...
sudo echo "export" NEO4J_INIT_HOME=`pwd` | sudo tee -a /etc/profile.d/insightsenvvar.sh
. /etc/environment
. /etc/profile.d/insightsenvvar.sh
sleep 10
sudo chmod -R 777 /opt/NEO4J_HOME
cd /etc/init.d/
sudo wget https://raw.githubusercontent.com/CognizantOneDevOps/Insights/master/PlatformDeployment/RHEL7/initscripts/Neo4j.sh
