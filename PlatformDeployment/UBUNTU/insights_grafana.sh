...
echo -n "credentials: "
read grafanacreds
sed -i "/type =/ s/=.*/=postgres/" defaults.ini
sed -i "/host =/ s/=.*/=localhost:5432/" defaults.ini
sed -i "/name =/ s/=.*/=grafana/" defaults.ini
sed -i "/user =/ s/=.*/=grafana/" defaults.ini
sed -i "/password =/ s/=.*/=$grafanacreds/" defaults.ini
sed -i "/allow_loading_unsigned_plugins =/ s/=.*/=neo4j-datasource,Inference,cde-inference-plugin,cde-fusion-panel,cognizant-insights-charts/" defaults.ini
sed -i "/allow_embedding =/ s/=.*/=true/" defaults.ini
cd /opt/grafana

export GRAFANA_HOME=`pwd`
sudo echo GRAFANA_HOME=`pwd` | sudo tee -a /etc/environment
sudo echo "export" GRAFANA_HOME=`pwd` | sudo tee -a /etc/profile.d/insightsenvvar.sh
. /etc/environment
. /etc/profile.d/insightsenvvar.sh
