...
class PipelinePanelCtrl extends MetricsPanelCtrl {
  ...
  //pipelineToolsArray = ['JIRA', 'BITBUCKET', 'JENKINS', 'SONAR', 'RUNDECK', 'TESTING', 'LOADRUNNER'];
  ...
  render() {
    ...
    this.pipelineToolsArray = this.panel.toolsInsightsPanelCtrl.selectedToolsSeq;
  ...
  parseQueryResult(queryData): void {
    ...
    for (var key in queryData.results) {
      ...
      for (let i in toolsResultSet) {
        ...
        toolDataMap[toolData.name] = toolData;
      }
      for (var toolNm of this.pipelineToolsArray) {
        if (toolDataMap[toolNm] !== undefined) {
          this.toolsRelationDataArray.push(toolDataMap[toolNm]);
        }
      }
    }
