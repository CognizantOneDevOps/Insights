...
public class AuditServiceUtil {
	...
	// GetAssetDetails and QueryByDate: Convert the output from ledger into format
	// suitable for UI
	public static JsonObject parseOutput(String message) {
		...
		try {
			formattedresponse.addProperty(InsightsAuditConstants.STATUS_CODE,
					inputMessage.getAsJsonPrimitive(InsightsAuditConstants.STATUS_CODE).getAsString());
			if (!inputMessage.getAsJsonPrimitive(InsightsAuditConstants.STATUS_CODE).getAsString().equals("200")) {
				formattedresponse.addProperty(InsightsAuditConstants.DATA,
						inputMessage.getAsJsonPrimitive(InsightsAuditConstants.MSG).getAsString());
			} else {
				addProperty(inputMessage, formattedresponse);
			}
		...
		try {
			...
			if (inputMessage.get(InsightsAuditConstants.MSG).isJsonArray()) {
				for (JsonElement entry : inputMessage.getAsJsonArray(InsightsAuditConstants.MSG)) {
					messageAddProperty(array, entry);
				}
				...
				if (array.size() > 0) {
					...
					addJsonObject(formattedresponse, aL);
				} else {
	...
	private static void addJsonObject(JsonObject formattedresponse, ArrayList<JsonObject> aL) {
		JsonArray sortedArray = new JsonArray();
		for (JsonObject entry : aL) {
	...
	private static void messageAddProperty(JsonArray array, JsonElement entry) {
		...
		for (Map.Entry<String, JsonElement> entrySet : entry.getAsJsonObject().getAsJsonObject("Value").entrySet()) {
			...
			} else if (entrySet.getKey().equals(InsightsAuditConstants.UPLINK)
					|| entrySet.getKey().equals(InsightsAuditConstants.DOWNLINK)) {
				continue;
			} else {
	...
	private static void addProperty(JsonObject inputMessage, JsonObject formattedresponse) throws Exception {
		if (inputMessage.get(InsightsAuditConstants.MSG).isJsonArray()) {
			JsonArray array = new JsonArray();
			...
			if (array.size() > 0)
				...
				formattedresponse.addProperty(InsightsAuditConstants.STATUS_CODE, "104");
				formattedresponse.add(InsightsAuditConstants.DATA,
						new JsonPrimitive("No assets found within the selected date range"));
			}
		} else {
			JsonArray array = new JsonArray();
