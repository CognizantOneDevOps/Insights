...
public class ReportManagementTest extends LoginAndSelectModule {
	...
	public void setUp() {
		...
		selectModuleUnderConfiguration("Report Management");
	 ...
	 Assert.assertTrue(assessmentReportPage.addNewReport(),
	  "Not able to add Report");
	 ...
	 Assert.assertTrue(assessmentReportPage.createDuplicateReport(),
	"Report with same name already exists.");
	 
	  Assert.assertTrue(assessmentReportPage.navigateToReportsLandingPage(),
	  "ReportName already exists");
	  }
		/*
		 * @Test(priority = 3) public void updateReport() throws InterruptedException {
		 * 
		 * Assert.assertTrue(assessmentReportPage.editReport(),
		 * "Not able to navigate to landing page");
		 * 
		 * }
		 */
