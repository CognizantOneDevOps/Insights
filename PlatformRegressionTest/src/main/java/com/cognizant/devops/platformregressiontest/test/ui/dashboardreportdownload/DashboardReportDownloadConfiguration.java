...
public class DashboardReportDownloadConfiguration extends DashboardReportDownloadObjectRepository {
	...
	public boolean editDashboardReport(DashboardReportDataModel data) {
		try {
			...
			log.info("Failed to edit the report.");
			return false;
	 ...
	 * @return
	 */
	public boolean checkDetailsFunctionality(String title) {
		selectReport(title);
		try {
			driver.findElement(
					By.xpath("//tr/td[contains(text(), '" + title + "')]//following-sibling::td[6]")).click();
			visibilityOf(workfloHistoryDetail, 2);
			if (workfloHistoryDetail.getText().equals("Workflow History Detail - " + title)) {
				closeDialog.click();
			}
		} catch (Exception e) {
			closeDialog.click();
	 ...
	 */
	private void fillData(DashboardReportDataModel data) throws InterruptedException {
