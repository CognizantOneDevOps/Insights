...
public class MileStoneApiTestCases extends OutcomeTest{
	
	private static final Logger log = LogManager.getLogger(OutcomeApiTestCases.class);

	@Test(priority = 1)
	public void fetchMileStoneConfig() {

		RestAssured.baseURI = CommonUtils.getProperty("baseURI") + CommonUtils.getProperty("fetchMileStoneConfig");
		// Response Object
        Response getResponse =setHeaders(null,"GET");

		String responseMile = getResponse.getBody().asString();
		log.debug("SuccessResponse {}", responseMile);

		int statusCode = getResponse.getStatusCode();

		log.debug("StatusCode {}", statusCode);
		Assert.assertEquals(statusCode, 200);
		Assert.assertEquals(responseMile.contains("success"), true);
	}
	@Test(priority = 2,dataProvider = "milestonedataprovider")
	public void saveMileStoneConfig(String config,String configUpdate) {

		RestAssured.baseURI = CommonUtils.getProperty("baseURI") + CommonUtils.getProperty("saveMileStoneConfig");
		// Response Object
        Response getResponse =setHeaders(config,"POST");

		String responseMileStone = getResponse.getBody().asString();
		log.debug("SuccessResponse {}", responseMileStone);

		int statusCode = getResponse.getStatusCode();

		log.debug("StatusCode {}", statusCode);
		Assert.assertEquals(statusCode, 200);
		Assert.assertEquals(responseMileStone.contains("success"), true);
	}
	@Test(priority = 3,dataProvider = "milestonedataprovider")
	public void updateMileStoneConfig(String config,String configUpdate) {

		RestAssured.baseURI = CommonUtils.getProperty("baseURI") + CommonUtils.getProperty("updateMileStoneConfig");
		// Response Object
        Response getResponse =setHeaders(configUpdate,"POST");
