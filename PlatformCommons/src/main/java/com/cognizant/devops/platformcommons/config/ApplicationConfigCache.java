...
public class ApplicationConfigCache {
	 ...
	 * @return true if all property of ApplicationConfigProvider initialize correctly 
	 * @throws Exception 
	 */
	 ...
	 * @param json
	 * @return true if all property of ApplicationConfigProvider initialize correctly/successfully 
	 */
	...
	public static boolean initializeUsingJson(JsonObject json) {
		try {
			Gson gson = new Gson();
			if (json != null) {
				ApplicationConfigProvider config = gson.fromJson(json,
	...
	/** Save servre config file on file system and Initialize ApplicationConfigProvider object 
	...
	public static boolean validateServerConfig() {
		...
		try {
			...
			Set<ConstraintViolation<ApplicationConfigProvider>> violations = validator.validate(ApplicationConfigProvider.getInstance());
			violations.forEach(g -> 
				log.error("Error while validating server config json === {} ===== {}  ", g.getMessage(),g)
			);
