...
public class ElasticSearchDBHandler {
	...
	public JsonObject queryES(String sourceESUrl,String query) throws Exception{
		...
		try {
			...
			if(response.getStatus() != 200) {
				throw new Exception("Failed to get response from ElasticSeach for query - "+query+ "-- HTTP response code -"+response.getStatus());
			}
	...
	public static void main(String[] args) throws Exception{
		...
		JsonObject jsonObj = elasticSearchDBHandler.queryES("http://127.0.0.1:9200/spark-jobs-conf/results/_search?size=2&filter_path=hits", esQuery);
		...
		for(JsonElement element : array){
			...
			//resultMap.put(kpiDefinition.getName(), Long.valueOf(element.getAsJsonObject().get("avg_duration").getAsJsonObject().get("value").getAsLong()));
			/*Long avg = Long.valueOf(output.get("avg_output").getAsJsonObject().get("value").getAsLong());
			String groupByValue = output.get("key").getAsString();*/
		...
		/*for(Map.Entry<String, JsonElement> confEntry : jsonObj.entrySet()){
			JsonElement value = confEntry.getValue();
				System.out.println(confEntry.getKey());	
				System.out.println(value.toString());
		}*/
		/*JsonArray asJsonArray = jsonObj.get("aggregations");
		for(JsonElement vizEle : asJsonArray){
			JsonObject viz = vizEle.getAsJsonObject();
