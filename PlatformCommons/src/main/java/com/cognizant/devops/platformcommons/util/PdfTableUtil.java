...
import com.cognizant.devops.platformcommons.util.LoadFile;
...
public class PdfTableUtil {
	...
	private void generatePageStaticContent(PDDocument doc, float margin, PDPage page, float yStartNewPage,
		...
		PageContentStreamOptimized pageContentStreamOptimized = new PageContentStreamOptimized(cos);
		PDStreamUtils.rect(pageContentStreamOptimized, 10, ystaticStart-65, page.getMediaBox().getWidth()-20, 1, Color.blue);
		cos.close();
...
//		File staticFile = new File(staticFilePath);
		...
		PDPageContentStream cos = new PDPageContentStream(doc, page);
		PageContentStreamOptimized pageContentStreamOptimized = new PageContentStreamOptimized(cos);
		PDStreamUtils.write(pageContentStreamOptimized, pdfStaticContent.getTitle(), PDType1Font.HELVETICA, 30, 80,
				yStart, Color.BLACK);
		PDStreamUtils.rect(pageContentStreamOptimized, 80, yStart - 35, page.getMediaBox().getWidth() - 230, 1,
				Color.blue);
