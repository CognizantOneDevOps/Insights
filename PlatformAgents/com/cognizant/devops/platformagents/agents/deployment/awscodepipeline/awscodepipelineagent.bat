...
for /f "delims=" %%i in ('python -V ^2^>^&^1') do (
   ...
   if "!PYTHON_VERSION:~0,8!" EQU "Python 2" ( 
	  ...
	  python -c "from __AGENT_KEY__.com.cognizant.devops.platformagents.agents.deployment.awscodepipeline.AwsCodePipeline import AwsCodePipeline; AwsCodePipeline()"
   ) else (
      if "!PYTHON_VERSION:~0,8!" EQU "Python 3" ( 
         echo Detected python 3 version
		 python -c "from __AGENT_KEY__.com.cognizant.devops.platformagents.agents.deployment.awscodepipeline.AwsCodePipeline3 import AwsCodePipeline; AwsCodePipeline()"
      ) else ( 
