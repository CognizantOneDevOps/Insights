...
case "$1" in
  start)
    if [[ $(ps aux | grep '[s]cm.bitbucketAllBranch.BitBucketAgentAllBranches' | awk '{print $2}') ]]; then
  ...
  stop)
    ...
    if [[ $(ps aux | grep '[s]cm.bitbucketAllBranch.BitBucketAgentAllBranches' | awk '{print $2}') ]]; then
     sudo kill -9 $(ps aux | grep '[s]cm.bitbucketAllBranch.BitBucketAgentAllBranches' | awk '{print $2}')
    else
