...
the License.
:comment
pushd %INSIGHTS_AGENT_HOME%\PlatformAgents\checkmarx
...
for /f "delims=" %%i in ('python -V ^2^>^&^1') do (
   set PYTHON_VERSION=%%i
      if "!PYTHON_VERSION:~0,8!" EQU "Python 3" ( 
		 ...
		 python -c "from  __AGENT_KEY__.com.cognizant.devops.platformagents.agents.codequality.checkmarx.CheckmarxAgent3 import CheckmarxAgent3; CheckmarxAgent3()"
      ) else ( 
