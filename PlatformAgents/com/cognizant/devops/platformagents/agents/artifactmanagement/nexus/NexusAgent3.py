...
class NexusAgent(BaseAgent):
    def process(self):
        ...
        for artifacts in range(len(nexIDs["data"])):
            ...
            req = urllib.request.Request(BaseUrl+"service/local/repositories/"+repoid+"/content/"+groupidrep+"/"+artifactid+"/"+version+"/maven-metadata.xml")
            with urllib.request.urlopen(req) as response:
                mavenmetadata = response.read()
            #mavenmetafile = urllib.request.urlopen(BaseUrl+"service/local/repositories/"+repoid+"/content/"+groupidrep+"/"+artifactid+"/"+version+"/maven-metadata.xml")
