...
detectPythonVersion()
{
     if echo "$1" | grep -q "Python 3"; then
      ...
      python -c "from __AGENT_KEY__.com.cognizant.devops.platformagents.agents.roi.splunk.SplunkAgent import SplunkAgent; SplunkAgent()" &
     else
...
case "$1" in
  start)
    if [[ $(ps aux | grep '__PS_KEY__' | awk '{print $2}') ]]; then
     ...
     cd $INSIGHTS_AGENT_HOME/PlatformAgents/splunk
	 echo $python_version
     detectPythonVersion "$python_version"
    fi
    if [[ $(ps aux | grep '__PS_KEY__' | awk '{print $2}') ]]; then
     echo "InSi__AGENT_KEY__ rted Sucessfully"
    else
