...
detectPythonVersion()
{
     if echo "$1" | grep -q "Python 3"; then
      ...
      python -c "from __AGENT_KEY__.com.cognizant.devops.platformagents.agents.roi.newrelic.NewRelicAgent import NewRelicAgent; NewRelicAgent()" &
     else
...
case "$1" in
  start)
    if [[ $(ps aux | grep '__PS_KEY__' | awk '{print $2}') ]]; then
     ...
     cd $INSIGHTS_AGENT_HOME/PlatformAgents/newrelic
	 echo $python_version
