...
detectPythonVersion()
{
     if echo "$1" | grep -q "Python 3"; then
      ...
      python -c "from __AGENT_KEY__.com.cognizant.devops.platformagents.agents.roi.elasticsearch.ElasticSearchAgent import ElasticSearchAgent; ElasticSearchAgent()" &
     else
...
case "$1" in
  start)
    if [[ $(ps aux | grep '__PS_KEY__' | awk '{print $2}') ]]; then
     ...
     cd $INSIGHTS_AGENT_HOME/PlatformAgents/elasticsearch
	 echo $python_version
     detectPythonVersion "$python_version"
    fi
    if [[ $(ps aux | grep '__PS_KEY__' | awk '{print $2}') ]]; then
     echo "InSights __AGENT_KEY__ started Sucessfully"
    else
