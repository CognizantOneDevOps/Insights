...
#-------------------------------------------------------------------------------
#! /bin/sh
# /etc/init.d/InSightsBambooAgent
...
case "$1" in
  start)
    if [[ $(ps aux | grep '[c]i.bamboo.BambooAgent' | awk '{print $2}') ]]; then
  ...
  stop)
    ...
    if [[ $(ps aux | grep '[c]i.bamboo.BambooAgent' | awk '{print $2}') ]]; then
     sudo kill -9 $(ps aux | grep '[c]i.bamboo.BambooAgent' | awk '{print $2}')
    else
  ...
  status)
    ...
    ;;
  *)
    echo "Usage: /etc/init.d/InSightsBambooAgent {start|stop|restart|status}"
    exit 1
    ;;
esac
exit 0
