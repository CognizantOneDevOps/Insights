...
#-------------------------------------------------------------------------------
#! /bin/sh
# /etc/init.d/InSightsDynatraceAgent
...
case "$1" in
  start)
    if [[ $(ps aux | grep '[a]ppmonitoring.Dynatrace.DynatraceAgent' | awk '{print $2}') ]]; then
  ...
  stop)
    ...
    if [[ $(ps aux | grep '[a]ppmonitoring.dynatrace.DynatraceAgent' | awk '{print $2}') ]]; then
     sudo kill -9 $(ps aux | grep '[a]ppmonitoring.dynatrace.DynatraceAgent' | awk '{print $2}')
    else
  ...
  status)
    ...
    ;;
  *)
    echo "Usage: /etc/init.d/InSightsDynatraceAgent {start|stop|restart|status}"
    exit 1
    ;;
esac
exit 0
